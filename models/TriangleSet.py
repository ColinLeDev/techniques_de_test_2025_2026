...
class TriangleSet:
    ...
    def to_bytes(self) -> bytes:
        ...
        ts_body = b"".join(struct.pack('<III', t.p1, t.p2, t.p3) for t in self.triangles)
    ...
    def from_bytes(cls, data: bytes) -> 'TriangleSet':
        ...
        if len(data) < ps_size + 4:
            if len(data) == ps_size:
                raise ValueError("It seems like the data contains only a PointSet without TriangleSet")
